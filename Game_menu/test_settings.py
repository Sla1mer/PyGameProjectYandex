...
def change_window(screen_size, flag=None):
    global screen
    if flag is None:
...
def options(screen_size, screen):
    ...
    all_settings_button.add(
        Button(volume_ico_size[0] + 25 + 80, volume_ico_size[1] + 25, 'change_volume(0.1)',
               'plus.png', (50, 50)))
    all_settings_button.add(
        Button(button_pos_x, button_pos_y * 4, 'change_window(screen_size, 1)', 'button_change_screen1.png',
               button_size))
    all_settings_button.add(
    ...
    while running:
        ...
        draw_text(f'текущая громкость {round(get_volume() * 100)}%', screen_size[0] // 72, (255, 255, 0),
